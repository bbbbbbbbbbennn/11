...
local function AutoPickItem()
    ...
    if not foundItem then
        ShowNotification("未找到印钞，准备切换服务器")
        task.wait(1)
        TPServer()
    else
        ShowNotification("找到印钞，重新搜索")
        task.wait(0.1)
        AutoPickItem()
    end
end
...
local function TPServer()
    ...
    if not success then
        ShowNotification("获取服务器列表失败，等待3秒后重试")
        task.wait(3)
        return TPServer()
    end
    ...
    if #availableServers > 0 then
        ShowNotification("找到"..#availableServers.."个可用服务器，正在切换...")
        TeleportService:TeleportToPlaceInstance(game.PlaceId, availableServers[math.random(1, #availableServers)])
    else
        ShowNotification("没有可用服务器，等待1秒后重试")
        task.wait(1)
        TPServer()
    end
end
